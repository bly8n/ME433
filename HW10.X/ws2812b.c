...
int main(void) {
    ...
    float hue = 0.0;  // Starting hue value

    while (1) {
        // Generate colors for each light based on the current hue
        for (int i = 0; i < 5; i++) {
            c[i] = HSBtoRGB(hue, 1.0, 0.25);
            hue += 10.0;  // Increase the hue value for the next light
            if (hue > 360.0) {
                hue -= 360.0;  // Wrap around the hue value if it exceeds 360
            }
        }

        ws2812b_setColor(c, 5);

        // Add a delay to control the speed of the color flow
    ...
    //c[0] = HSBtoRGB(0.0, 1.0, 0.25); // red
    //c[1] = HSBtoRGB(60.0, 1.0, 0.3); // green
    //c[2] = HSBtoRGB(240.0, 1.0, 0.4); // blue
    //c[3] = HSBtoRGB(120.0, 1.0, 0.5); // yellow
    //c[4] = HSBtoRGB(180.0, 1.0, 0.6);
    //ws2812b_setColor(c,5);