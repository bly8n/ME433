...
def webpage():
    ...
    <script>
        ...
        function stopwatch() {{
            let now = Date.now();
            if (lastPressed !== null) {{
                let elapsed = now - lastPressed;
                document.getElementById('elapsed-time').textContent = elapsed / 1000;
            }}
            lastPressed = now;
        }}
    </script>
    ...
    <button class="button" onclick="stopwatch()" type="button">Stopwatch</button>
    <p id="elapsed-time">0</p>
    </body></html>
    """
    return html
