...
while True:
    ...
    for i in range(0,60):
       pixel_value = bitmap[row,i]
    # Extract red, green, and blue components from the RGB565 pixel
       red = ((pixel_value >> 11) & 0x1F) * (100 / 31)
       green = ((pixel_value >> 5) & 0x3F) * (100 / 63)
       blue = (pixel_value & 0x1F) * (100 / 31)
    # Store the color intensities
       reds[i] = red
       greens[i] = green
       blues[i] = blue
    # Calculate brightness as the sum of color intensities
       bright[i] = red + green + blue
